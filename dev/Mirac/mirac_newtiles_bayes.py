...
def main (main_paramaters):
  alpha, gamma = main_paramaters
  ...
  for episode in range(2000):
    ...
    if TRAIN:
      if episode_num % 5 == 0:
        q = np.array(qvals)
        filename = "q_erdos_queristiclearning_act4_2K_bayes_gamma_{}_alpha_{}_weights_16_3_2_1.npy".format(gamma, alpha)  
        np.save(filename, q)
...
if __name__ == '__main__':
  ...
  for _ in range(10):
    ...
    with open("mirac_bayes.txt","w") as f:
